...
const items = ref([
  {
    id: 101,
    title: 'Server Health Inspection',
    status: 'ACTIVE',
    priority: 'HIGH'
  },
  {
    id: 102,
    title: 'Prisma DB Migration',
    status: 'ACTIVE',
    priority: 'CRITICAL'
  },
  {
    id: 103,
    title: 'TanStack Query Sync',
    status: 'PENDING',
    priority: 'MEDIUM'
  },
...
<template>
  <div class="space-y-6 max-w-4xl mx-auto w-full">
    <!-- Header -->
    <Card
      class="p-6 md:p-8 space-y-4 bg-gradient-to-br from-purple-500/10 via-primary/5 to-background border-purple-500/30"
    >
      <div class="flex items-center gap-2">
        <Badge
          variant="outline"
          class="bg-purple-500/10 text-purple-500 border-purple-500/30 font-bold"
        >
          🚥 Template Directives & Control Flow
        </Badge>
        <span class="text-xs font-mono text-muted-foreground"
          >v-if, v-for, :key Reconciliation</span
        >
      </div>
      ...
      <p class="text-sm text-muted-foreground leading-relaxed">
        Test conditional branch rendering (<code
          class="text-primary font-mono font-bold"
          >v-if / v-else-if / v-else</code
        >), dynamic list rendering with stable
        <code class="text-primary font-mono font-bold">:key</code>
        reconciliation, and invisible
        <code class="text-primary font-mono font-bold">&lt;template&gt;</code>
        wrapper tags.
      </p>
    </Card>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <Card class="p-5 md:p-6 space-y-4">
        <h3 class="text-base font-bold text-foreground">
          1. Conditional Branching (<code class="font-mono text-primary"
            >v-if</code
          >)
        </h3>
        ...
        <!-- Dynamic Output -->
        <div
          class="p-4 rounded-xl border border-border bg-accent/20 min-h-28 flex flex-col justify-center"
        >
          <div v-if="activeTab === 'ADMIN'" class="space-y-1">
            <span class="text-xs font-bold text-emerald-500"
              >🛡️ Administrator Clearance</span
            >
            <p class="text-xs text-muted-foreground">
              Full CRUD permissions, database migration access, and tenant
              administration enabled.
            </p>
          </div>
          <div v-else-if="activeTab === 'MANAGER'" class="space-y-1">
            <span class="text-xs font-bold text-blue-500"
              >👔 Manager Clearance</span
            >
            <p class="text-xs text-muted-foreground">
              Department moderation, analytics overview, and task management
              enabled.
            </p>
          </div>
          <div v-else class="space-y-1">
            <span class="text-xs font-bold text-amber-500"
              >👤 Guest Clearance</span
            >
            <p class="text-xs text-muted-foreground">
              Read-only preview mode. Please sign in to modify records.
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card class="p-5 md:p-6 space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="text-base font-bold text-foreground">
            2. List Diffing (<code class="font-mono text-primary"
              >v-for :key</code
            >)
          </h3>
          <Button size="sm" variant="primary" @click="addItem"
            >+ Add Row</Button
          >
        </div>
        ...
        <div class="space-y-2 max-h-56 overflow-y-auto pr-1">
          ...
          >
            ...
            <Button
              size="sm"
              variant="ghost"
              class="text-rose-500 text-xs px-2 h-7"
